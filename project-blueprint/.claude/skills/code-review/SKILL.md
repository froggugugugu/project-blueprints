...
- 見つけた指摘は**全件列挙し、重要度で絞る**。「重大なものだけ報告」「控えめに」とは指示しない(Claude 5 系は文字どおり報告を減らす)。
  対応が必須なのは正確性・要件・プロジェクト規約に影響する MUST だけ。SHOULD / CONSIDER まで全件対応させると過剰設計を招く(公式ガイド)
- 見つけた問題を「大したことはない」と自分で却下しない。判定は重要度定義に従い、迷ったら SHOULD に残す(自己評価は甘くなる — 公式知見)
