...
`references/ui-glossary-template.md` を参照。

---

### Phase 2: HIG準拠チェック(8カテゴリ)

`references/hig-check-categories.md` に定義された8カテゴリ(A: ボタン・アクション / B: アイコン / C: ナビゲーション / D: フォーム・入力 / E: フィードバック・状態表示 / F: タイポグラフィ / G: レイアウト・構造 / H: アクセシビリティ)を**サブエージェントで並行スキャン**する。各カテゴリのチェック項目・検出方法・HIG根拠は参照ファイルを参照。
