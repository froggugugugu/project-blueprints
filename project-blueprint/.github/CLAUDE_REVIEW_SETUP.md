...
| モデル | 用途 | コスト目安 |
| ------ | ---- | ---------- |
| `claude-opus-4-7` | 高品質レビュー、アーキテクチャ監査 | 高（$0.30〜/PR） |
| `claude-sonnet-4-6` | 標準レビュー（推奨） | 中（$0.10/PR） |
| `claude-haiku-4-5` | 軽量レビュー、簡易チェック | 低（$0.02/PR） |
...
- **コードが大きい PR**: `max_turns` を一時的に増やすより、事前に `/split-pr` のような分割戦略を推奨
